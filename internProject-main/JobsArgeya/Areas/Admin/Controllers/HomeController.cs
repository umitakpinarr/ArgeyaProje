...
namespace JobsArgeya.Areas.Admin.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            int UserId = Convert.ToInt32(User.Claims.FirstOrDefault(d => d.Type == "OfficeId").Value);
            List<Users> DbUsers = _databaseContext.Users.Where(b => b.RoleId == UserId).ToList();
            List<AppliesViewModel> AllApplies = new List<AppliesViewModel>();
            if(UserId == 2)
            {
                DbApplies = _databaseContext.Applies.ToList();
            }
